...
# Define 10 filler words/phrases that end with punctuation
FILLER_WORDS = [
    "um.",
    "uh.",
    "hmm.",
...
class WebSocketServer:
    ...
    async def broadcast(self, message: str):
        if self.connected:
            ...
            await asyncio.gather(*(ws.send(data) for ws in self.connected))
...
#########################################
# Audio Streaming & Playback (Optimized)
#########################################
class AudioStreamPlayer:
    ...
    def play_audio_stream(self):
        while True:
            ...
            # No sleep needed since queue.get() blocks

#########################################
# Edge-TTS Conversion Functions (Optimized)
#########################################
async def async_text_to_speech(text: str, voice: str = TTS_VOICE, rate: int = TTS_RATE, pitch: int = TTS_PITCH) -> bytes:
    if not text.strip():
        return None
    rate_str = f"{rate:+d}%"
...
def text_to_speech_sync(text: str, voice: str = TTS_VOICE, rate: int = TTS_RATE, pitch: int = TTS_PITCH) -> bytes:
    return asyncio.run(async_text_to_speech(text, voice, rate, pitch))

#########################################
# TTS Streaming Thread (Optimized)
#########################################
def stream_to_edge_tts(text_queue: queue.Queue, audio_player: AudioStreamPlayer):
    accumulated_text = ""
    while True:
        try:
            # Block until text is available (with a timeout)
            text_chunk = text_queue.get(timeout=0.1)
            if not isinstance(text_chunk, str):
                text_chunk = str(text_chunk)
            accumulated_text += text_chunk
            # When a sentence-ending punctuation is reached, process TTS conversion
            if accumulated_text.strip() and accumulated_text.strip()[-1] in ".!?":
                try:
                    audio_data = text_to_speech_sync(accumulated_text)
                    if audio_data:
...
#########################################
# OpenAI Chat Generation (Streaming)
#########################################
...
#########################################
# Conversation System Class with WebSocket Integration (Optimized)
#########################################
class ConversationSystem:
    def __init__(self):
        ...
        # Start audio playback thread (uses blocking queue calls)
        ...
        # Start the TTS conversion thread
        ...
        # Precompute filler audio for faster response times
        self.filler_audio_cache = {}
        for filler in FILLER_WORDS:
            try:
                audio_data = text_to_speech_sync(filler)
                self.filler_audio_cache[filler] = audio_data
            except Exception as e:
                print(f"Error generating filler audio for '{filler}': {e}")
                ...
                try:
                    ...
                    try:
                        ...
                        # Process user input if awake
                            ...
                            # Insert a random filler word using the cached audio for a faster response.
                            filler = random.choice(FILLER_WORDS)
                            if filler in self.filler_audio_cache:
                                self.audio_player.add_audio_chunk(self.filler_audio_cache[filler])
                            else:
                            ...
                            # Start OpenAI streaming in a new thread.
