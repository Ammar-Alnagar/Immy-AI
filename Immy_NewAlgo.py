...
def send_to_openai_streaming(user_input: str, text_queue: queue.Queue, conversation_history: list, history_lock: threading.Lock) -> None:
    ...
    # Immediately acknowledge the user input with a random filler word.
    filler_word = random.choice(FILLER_WORDS)
    text_queue.put(filler_word)
    sys.stdout.write(filler_word + "\n")
    sys.stdout.flush()
    ...
    try:
        ...
        answer_text = ""
        accumulated_text = ""
        # Flush as soon as at least 5 words are accumulated or if 0.5 seconds have passed since last flush.
        CHUNK_WORD_THRESHOLD = 30
        FLUSH_DELAY = 0.5  # seconds
        ...
        for chunk in stream:
            if chunk.choices[0].delta.content is not None:
                ...
                now = time.time()
                # Flush if we've reached the word threshold or if a short delay has passed.
                if (len(accumulated_text.strip().split()) >= CHUNK_WORD_THRESHOLD) or ((now - last_flush_time) > FLUSH_DELAY and accumulated_text.strip()):
            ...
            while self.should_run:
                ...
                try:
                    ...
                    if not self.is_awake and WAKE_WORD in text:
                        ...
                        # Spawn a thread to handle streaming of the answer.
